...
class Filtro extends Component {
  constructor(props) {
    ...
    this.TodoslosAlojamientos = this.TodoslosAlojamientos.bind(this);
  ...
  handleActivo = (id, event) => {
    const copy_servicios = Object.assign([], this.state.servicios).map(
      (currentValue) => {
  ...
  findValorizacion = (idTipoValorizacion) => {
    if (idTipoValorizacion === 0) {
      ...
      fetch(
        `${process.env.REACT_APP_URL_API_SERVER_2}/valorizaciones/tipo/${idTipoValorizacion}`,
        {
          headers: new Headers({
        ...
        if (res.ok && res.status === 200) {
          ...
          this.setState({
            ...
            valorizacion: {
              registros: [
                { id: 0, descripcion: "Error al obtener los datos!" },
              ],
              selected: 0,
            },
          });
  ...
  findCiudades = (idDepartamento) => {
    if (idDepartamento === 0) {
      ...
      fetch(
        `${process.env.REACT_APP_URL_API_SERVER_2}/ciudades/departamento/${idDepartamento}`,
        {
          headers: new Headers({
            Authorization: localStorage.getItem("WebTurToken"),
          }),
  ...
  componentDidMount() {
    ...
    this.TodoslosAlojamientos();
    //Obtener todos los Departamentos
    let departamentos = new Promise((acept, reject) => {
      fetch(
        `${process.env.REACT_APP_URL_API_SERVER_2}/departamentos/provincia/1`,
        {
          method: "GET",
          headers: new Headers({
            Authorization: localStorage.getItem("WebTurToken"),
    ...
    Promise.all([departamentos, tipos, tiposdevalorizacion, servicios]).then(
      (values) => {
        this.setState({
          loading: false,
        });
  ...
  render() {
    ...
    const departamentos = this.state.departamentos.registros.map((depto) => {
      return (
        <option key={`Dep-${depto.id}`} value={depto.id}>
          {depto.nombre}
        </option>
      );
    });
    const ciudades = this.state.ciudades.registros.map((ciudad) => {
      return (
        <option key={`City-${ciudad.id}`} value={ciudad.id}>
          {ciudad.nombre}
        </option>
      );
    });
    ...
    const tvalorizacion = this.state.tvalorizacion.registros.map((tval) => {
      return (
        <option key={`TipoV-${tval.id}`} value={tval.id}>
          {tval.descripcion}
        </option>
      );
    });
          ...
          <div className="input-group mb-2">
            <div className="input-group-prepend">
              <div className="input-group-text">
                <input
                  type="checkbox"
                  name={`ServCheck-${ser.id}`}
                  id={`ServCheck-${ser.id}`}
                  checked={ser.activo}
                  onChange={(e) => this.handleActivo(ser.id, e)}
                />
              </div>
            </div>
            ...
            <div className="input-group-append">
              <div className="input-group-text">
                <input
                  ...
                  name={`ServCant-${ser.id}`}
                  id={`ServCant-${ser.id}`}
                  value={ser.cantidad}
                  onChange={(e) => this.handleCantidad(ser.id, e)}
                />
              </div>
            </div>
          </div>
              ...
              <div className="col bg-white text-dark p-4 rounded">
                ...
                >
                    ...
                    <ExcelFile
                      element={
                        <button type="submit" 
                                ...
                                onClick={this.TodoslosAlojamientos}>
                          Exportar a excel los Alojamientos
                        </button>
                      }
                      filename="Excel Alojamientos "
                    >
                      <ExcelSheet data={this.state.guias} name="Pagina1">
                      <ExcelColumn
                      ...
                      />
                      <ExcelColumn
                        label="Adhiere a Covid"
                        value="adhiereCovid"
                      />
                      <ExcelColumn
                        label="Adhiere a Dosep"
                        value="adhiereDosep"
                      />
                      </ExcelSheet>
                    </ExcelFile>
                  ...
                  <div className="row">
                    <div className="col-xs-12 col-md-3">
                      <div className="form-group">
                        ...
                        <select
                          ...
                          name="idDepartamento"
                          id="idDepartamento"
                          value={this.state.departamentos.selected}
                    ...
                    <div className="col-xs-12 col-md-3">
                      <div className="form-group">
                        ...
                        <select
                          ...
                          name="idCiudad"
                          id="idCiudad"
                          value={this.state.ciudades.selected}
                    ...
                    <div className="col-xs-12 col-md-3">
                      <div className="form-group">
                        ...
                        <select
                          ...
                          name="idTipo"
                          id="idTipo"
                          value={this.state.tipos.selected}
                    ...
                    <div className="col-xs-12 col-md-3">
                      <div className="form-group">
                        <label htmlFor="idTipoValorizacion">
                          Tipo de Categoría
                        </label>
                        <select
                          className="form-control"
                          name="idTipoValorizacion"
                          id="idTipoValorizacion"
                          value={this.state.tvalorizacion.selected}
                          onChange={this.handleTValorizacionChange}
                        >
                          {tvalorizacion}
                        </select>
                      </div>
                    ...
                    <div className="col-xs-12 col-md-3">
                      <div className="form-group">
                        ...
                        <select
                          ...
                          name="idValorizacion"
                          id="idValorizacion"
                          value={this.state.valorizacion.selected}
                          onChange={this.handleValorizacionChange}
                        >
                          {valorizacion}
                        </select>
                      </div>
                  ...
                  <div className="row">{servicios}</div>
                  <div className="row">
                    <div className="col">
